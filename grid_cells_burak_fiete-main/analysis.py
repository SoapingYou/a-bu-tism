...
def blob_center(traces, smoothing = 2, verb=False, sort_centers = False):
    ...
    # start with the closest blob to the center
    pred[0] = centers[0, np.nanargmin(dist[0])]

    for i in range(1, t):
        d = np.linalg.norm(centers[i] - pred[i-1], axis=1)

        # ignore NaN blobs
        d[np.isnan(d)] = np.inf

        pred[i] = centers[i, np.argmin(d)]
...
def model_prediction(model_output, true_pos, silent=False, image_smoothing = 2, dt=1E-4, verb=True):
    ...
    print("best_center shape:", best_center.shape)
    print("best_center first:", best_center[:5])

    print("predicted_pos min:", predicted_pos.min(axis=0))
    print("predicted_pos max:", predicted_pos.max(axis=0))
    print("predicted_pos std:", predicted_pos.std(axis=0))

    print("true_pos min:", true_pos.min(axis=0))
    print("true_pos max:", true_pos.max(axis=0))
    print("true_pos std:", true_pos.std(axis=0))
